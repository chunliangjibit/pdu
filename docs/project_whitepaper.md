...
| **NM** | 6258/6260 **(-0.0%)** | 11.5/12.6 **(-8.7%)** | 3004/3600 **(-16.5%)** | 4.40/4.40 (0.0%) |
...
| **Tritonal** | 7169/6700 **(+7.0%)** | 18.3/25.0 **(-27%)** | 4233/3200 **(+32%)** | 10.23/7.50 (+36%) |
| **PBXN-109** | 8157/8050 (+1.3%) | 19.1/30.0 **(-36%)** | 2993/3300 (-9.3%) | 9.73/9.70 (+0.4%) |
...
### 3.1 核心改善与剩余挑战
1.  **硝基甲烷 (NM): 爆速误差收敛至 0.0%**
    *   **实现**: 通过引入 $r^*(\rho)$ 修正系数 $0.010$，有效补偿了液相分子的位阻效应，爆压从 11.2 GPa 回升至 11.5 GPa，爆速实现精准对接。
2.  **含铝炸药 (Tritonal): 动力学解耦成功**
    *   **实现**: Miller 两步法成功识别了爆轰面上的铝粉点火状态，爆速误差从 +14% 优化至 +7.0%，爆压实现了 1.3 GPa 的物理增量。
3.  **PETN: 爆温 T (-17.2%) 持续偏低**
    *   **深度分析**: Ree 修正虽然改善了压力，但可能显著改变了系统的能级分布（配分函数）。计划在 V10.2 中引入极性修正后的振动频率重校准。
